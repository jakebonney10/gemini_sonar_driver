...
## Key Differences from Raw GeminiComms Data

| Aspect | GeminiComms (Evo) | Svs5Sequencer (GLF) |
|--------|-------------------|---------------------|
| **Message Type** | CGemPingHead/Line/Tail | GLogTargetImage |
| **Data State** | Raw beam samples | Beamformed image |
| **Assembly** | Manual (3 messages) | Single complete image |
| **Bearing Angles** | Calculate from index | Provided in bearing table |
| **Metadata** | Extracted from headers | Comprehensive in structure |
| **Compression** | None | Optional (zlib/H264) |
| **Acoustic Zoom** | Not available | Included if enabled |

## Performance Considerations

### Memory

- **GLF images are larger**: Complete processed images vs raw samples
- **Bearing table**: Additional `num_beams * sizeof(double)` bytes
- **Metadata overhead**: ~100 bytes per ping

### Processing

- **No beamforming needed**: Data is already formed
- **Direct access**: All data in single structure
- **Simpler logic**: No need to accumulate multiple messages

## Example: Complete GLF Handler
...
void processGLFImage(const GLF::GLogTargetImage& image)
{
    ...
    if (!mainImage.m_vecData || !mainImage.m_vecBearingTable) {
        RCLCPP_WARN(logger, "Invalid GLF data");
        return;
    }
    ...
    for (size_t i = 0; i < num_beams; ++i) {
        size_t start = i * samples_per_beam;
        beams.emplace_back(
            mainImage.m_vecData->begin() + start,
            mainImage.m_vecData->begin() + start + samples_per_beam
        );
    }
    ...
    params.bins_per_beam = samples_per_beam;
    params.frequency_khz = mainImage.m_uiModulationFrequency / 1000.0;
    params.sound_speed_ms = mainImage.m_fSosAtXd;
    params.range_m = calculateRange(mainImage);
    
    // 5. Publish
    publishMessages(beams, params);
}
```
...
**Problem**: `m_vecData` or `m_vecBearingTable` is nullptr or empty

**Solutions**:
- Check sonar is online: `Svs5SetConfiguration(SVS5_CONFIG_ONLINE, ...)`
- Verify sonar discovery completed
- Check network connectivity
- Ensure configuration was sent successfully

### Wrong Dimensions

**Problem**: `samples_per_beam` calculation gives fractional result

**Solutions**:
- GLF data might be compressed - check `m_usCompressionType`
- Call `UncompressData()` if needed
- Verify image is complete (not corrupted)

### Missing Bearing Table

**Problem**: Bearing table is empty but data exists

**Solutions**:
- This shouldn't happen with properly formed GLF
- Check SDK version compatibility
- Verify GLF structure size matches
...
**GLF provides processed, ready-to-use sonar images** with comprehensive metadata. Compared to raw GeminiComms data:

✅ **Simpler**: Single message vs three-part sequence  
✅ **Complete**: All beam data and metadata together  
✅ **Precise**: Actual beam angles from beamformer  
✅ **Rich**: Platform info, chirp status, compression, zoom  

The tradeoff is slightly larger message size and loss of access to raw pre-beamformed samples (which most applications don't need).
